...
export const MasterDatabaseView: React.FC<MasterDatabaseViewProps> = ({
  ...
  const invoiceGroups = React.useMemo(() => {
    const map = new Map<string, { count: number; totalQty: number; items: MasterInvoiceItem[] }>();
    for (const item of items) {
      const existing = map.get(item.invoiceNo) || { count: 0, totalQty: 0, items: [] };
      existing.count += 1;
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 sm:p-5 shadow-lg space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-white">Daily Master Data Explorer</h2>
              <p className="text-xs text-slate-400">
                100% offline IndexedDB cache of daily loaded invoice lines & quantities
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1 text-xs">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search master items by SKU, Name, or Invoice..."
          ...
          >
            <option value="ALL">All Invoices ({invoiceGroups.length})</option>
            {invoiceGroups.map((g) => (
              <option key={g.invoiceNo} value={g.invoiceNo}>{g.invoiceNo}</option>
            ))}
          </select>
        </div>
      </div>
              ...
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item, index) => (
                  <tr key={item.id || index} className="hover:bg-slate-800/40 transition-colors">
                    <td className="p-3 pl-4 font-bold text-emerald-400">{item.invoiceNo}</td>
